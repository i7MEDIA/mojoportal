...
public static class DBModuleDefinition
{
	...
	)
	{
		...
		var sqlParams = new MySqlParameter[]
		{
			...
			new("?FeatureGuid", MySqlDbType.VarChar, 36) { Direction = ParameterDirection.Input, Value = featureGuid },
		};

		var count = Convert.ToInt32(MySqlHelper.ExecuteScalar(
			ConnectionString.GetReadConnectionString(),
			sqlCommand.ToString(),
			sqlParams).ToString()
		);
		...
		if (count > 0)
		{
			...
			sqlParams = [
				...
				new("?FeatureGuid", MySqlDbType.VarChar, 36) { Direction = ParameterDirection.Input, Value = featureGuid },
				new("?ResourceFile", MySqlDbType.VarChar, 255) { Direction = ParameterDirection.Input, Value = resourceFile },
				...
				new("?GroupName", MySqlDbType.VarChar, 255) { Direction = ParameterDirection.Input, Value = groupName },
				new("?Attributes", MySqlDbType.Text) { Direction = ParameterDirection.Input, Value = attributes },
				new("?Options", MySqlDbType.Text) { Direction = ParameterDirection.Input, Value = options },
				new("?Roles", MySqlDbType.VarChar, 255) { Direction = ParameterDirection.Input, Value = roles },
				new("?ShowToUnauthorized", MySqlDbType.Bit) { Direction = ParameterDirection.Input, Value = showToUnauthorized },
			];
	...
	)
	{
		...
		var sqlParams = new MySqlParameter[]
		{
			new("?ID", MySqlDbType.Int32) { Direction = ParameterDirection.Input, Value = id },
			new("?ModuleDefID", MySqlDbType.Int32) { Direction = ParameterDirection.Input, Value = moduleDefId },
			new("?SettingName", MySqlDbType.VarChar, 50) { Direction = ParameterDirection.Input, Value = settingName },
			new("?SettingValue", MySqlDbType.Text) { Direction = ParameterDirection.Input, Value = settingValue },
			new("?ControlType", MySqlDbType.VarChar, 50) { Direction = ParameterDirection.Input, Value = controlType },
