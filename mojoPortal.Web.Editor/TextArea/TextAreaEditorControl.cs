...
public class TextAreaEditorControl : Control, IPostBackDataHandler
{
	...
	protected override void Render(HtmlTextWriter writer) =>
		writer.Write($"""<textarea class="markituphtml" id="{ClientID}" name="{UniqueID}" rows="20" cols="80">{HttpUtility.HtmlEncode(Text)}</textarea>""");
	...
	{
		if (
			postCollection[postDataKey] is not null &&
