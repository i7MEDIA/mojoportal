...
#region contstructors
        ...
        private void MapDefinedMarkup(XmlNode node, bool isMobile = false)
        {
            if (node != null)
            {
                ...
                if (attrCollection["name"] != null) markupDefinitionName = attrCollection["name"].Value;
                ...
                if (attrCollection["jsonLabelObjects"] != null) jsonLabelObjects = Convert.ToBoolean(attrCollection["jsonLabelObjects"].Value);
                if (attrCollection["headerLocation"] != null) headerLocation = attrCollection["headerLocation"].Value;
                if (attrCollection["footerLocation"] != null) footerLocation = attrCollection["footerLocation"].Value;
                if (attrCollection["hideOuterWrapperPanel"] != null) hideOuterWrapperPanel = Convert.ToBoolean(attrCollection["hideOuterWrapperPanel"].Value);
                    ...
                    if (!String.IsNullOrWhiteSpace(childNode.InnerText) || !String.IsNullOrWhiteSpace(childNode.InnerXml))
                    {
                        switch (childNode.Name)
                        {
                            ...
                            case "RadioButtonListMarkup":
                                ...
                                if (cblmAttribs["field"] != null) cblm.Field = cblmAttribs["field"].Value;
                                if (cblmAttribs["token"] != null) cblm.Token = cblmAttribs["token"].Value;
