...
public partial class ViewSelectorSetting : UserControl, ICustomField
{
	...
	private void EnsureItems()
	{
		...
		themesPath = Path.Combine(SiteUtils.GetSkinPath(true), viewPath);
		globalThemesPath = Path.Combine("~/", viewPath);
		var themeFiles = GetLayouts(globalThemesPath);
	...
	private List<FileInfo> GetLayouts(string path)
	{
		...
		if (path.StartsWith("~/"))
		{
			dir = new(HttpContext.Current.Server.MapPath(path));
		}
		else
		{
