...
public partial class PageProperties : NonCmsBasePage
{
	...
	private void Page_Load(object sender, EventArgs e)
	{
		...
		if (pageId > -1)
		{
			...
			if (pageSettings.EditRoles == $"{Role.AdministratorsRole};")
			{
				if (!WebUser.IsAdmin)
				{
		...
		if (pageId == -1) //new page
		{
			...
			if (startPageId > -1)
			{
				...
				if (parentPage != null)
				{
					...
					pageSettings.StyleSets = parentPage.StyleSets;
					if (!IsPostBack)
					{
						hdnParentPageId.Value = parentPage.PageId.ToInvariantString();
						lblParentPageName.Text = parentPage.PageName;
	...
	private bool UserCanEdit()
	{
		return isAdminOrContentAdmin
			|| WebUser.IsInRoles(pageSettings.EditRoles)
			|| isSiteEditor
			|| WebUser.IsInRoles(siteSettings.RolesThatCanCreateRootPages)
			|| (parentPage is not null && WebUser.IsInRoles(parentPage.CreateChildPageRoles));
	}
	...
	{
		ddChangeFrequency.Items.AddRange(
		[
			new (Resource.PageChangeFrequencyAlways, "Always"),
			new (Resource.PageChangeFrequencyHourly, "Hourly"),
			new (Resource.PageChangeFrequencyDaily, "Daily"),
			new (Resource.PageChangeFrequencyWeekly, "Weekly"),
			new (Resource.PageChangeFrequencyMonthly, "Monthly"),
			new (Resource.PageChangeFrequencyYearly, "Yearly"),
			new (Resource.PageChangeFrequencyNever, "Never")
		]);
	}
	...
	private bool SavePageData()
	{
		...
		parentPage = new PageSettings(siteSettings.SiteId, newParentID);
