...
namespace mojoPortal.Web.UI
{
	...
	public partial class CmsPage : mojoBasePage
	{
		...
		private Control masterContainer = null;
		...
		private void LoadPage()
		{
			...
			if (UseGridLayout)
			{
				masterContainer = GridLayout;
			}
			else
			{
				masterContainer = MPContent;
			}
					...
					)
			{
				...
				masterContainer.Parent.Visible = true;
			}
			...
			if (
				WebConfigSettings.AutomaticallyAddCanonicalUrlToCmsPages &&
				Page.Header != null &&
				CurrentPage.UseUrl &&
			...
			foreach (Module module in CurrentPage.Modules)
			{
				...
				if (module.ControlSource == "Modules/LoginModule.ascx")
				{
					...
					masterContainer.Controls.Add(loginSettings); ///theme is not applied until its loaded
				...
				switch (module.PaneName.ToLower())
				{
					...
					case "altcontent1":
						...
						{
							log.Warn("Content is assigned to altcontent1 placeholder but it does not exist in layout.master so using center.");
							parent = masterContainer;
						}
					...
					case "altcontent2":
						...
						{
							log.Warn("Content is assigned to altcontent2 placeholder but it does not exist in layout.master so using center.");
							parent = masterContainer;
						}
				...
				// These are declared last so that that parent is only shown if the control is added
				parent.Visible = true;
