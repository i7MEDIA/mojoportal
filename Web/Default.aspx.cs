...
namespace mojoPortal.Web.UI
{
	...
	public partial class CmsPage : mojoBasePage
	{
		...
		private void LoadPage()
		{
			...
			if (siteSettings != null && CurrentPage != null && CurrentPage.EnableComments)
			{
				CommentsWidget comments = new CommentsWidget
				{
					CommentSystem = siteSettings.CommentProvider,
					SiteGuid = siteSettings.SiteGuid,
					CommentsClosedMessage = Resource.CommentsAreClosed,
					CommentUrl = SiteUtils.GetCurrentPageUrl(),
					IncludeIpAddressInNotification = true,
					RequireCaptcha = !Request.IsAuthenticated,
					EditBaseUrl = SiteRoot + "/Comments/CommentsDialog.aspx?pageid=" + CurrentPage.PageId.ToInvariantString(),
					SiteRoot = SiteRoot,
					UserCanModerate = WebUser.IsInRoles(CurrentPage.EditRoles),
					Visible = true,
					RequireAuthenticationToPost = true,
					UseCommentTitle = false,
					ShowUserUrl = false,
					SortDescending = true
				};

				// TODO: Add site settings for these
				...
				//comments.NotificationTemplateName = "CommentNotificationEmail.config";

				MPContent.Controls.Add(comments);

				//switch (siteSettings.CommentProvider)
				//{
				//	case "disqus":

				//		if (!WebConfigSettings.DisableExternalCommentSystems && siteSettings.DisqusSiteShortName.Length > 0)
				//		{
				//			DisqusWidget disqus = new DisqusWidget
				//			{
				//				SiteShortName = siteSettings.DisqusSiteShortName,
				//				WidgetPageUrl = WebUtils.ResolveServerUrl(SiteUtils.GetCurrentPageUrl())
				//			};
				...
				//		if (!WebConfigSettings.DisableExternalCommentSystems && siteSettings.IntenseDebateAccountId.Length > 0)
				//		{
				//			IntenseDebateDiscussion d = new IntenseDebateDiscussion
				//			{
				//				AccountId = siteSettings.IntenseDebateAccountId,
				//				PostUrl = SiteUtils.GetCurrentPageUrl()
				//			};
				...
				//	default:
				//	case "internal":
				//		var commentWidget = new CommentsWidget();

				//		MPContent.Controls.Add(commentWidget);
