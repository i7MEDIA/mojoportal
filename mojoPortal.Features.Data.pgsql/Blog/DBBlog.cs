...
namespace mojoPortal.Data
{
	public static class DBBlog
	{
		...
		public static IDataReader GetSingleBlog(int itemId, DateTime currentTime)
		{
			var sqlCommand = $"""
				SELECT
					...
					(
						SELECT b2.itemurl
						FROM mp_blogs b2
						WHERE b2.ispublished = true
						AND b2.startdate <= :currenttime
						AND (b2.enddate IS NULL OR b2.enddate > :currenttime)
						AND b2.startdate > b.startdate
						AND b2.moduleid = b.moduleid AND b2.itemurl IS NOT NULL
						AND b2.itemurl <> '' ORDER BY b2.startdate LIMIT 1
					) AS nextpost,
					(
						SELECT b2.title
						FROM mp_blogs b2
					...
					u.avatarurl,
					u.authorbio,
					string_agg(c.category, '{UnitSeparatorExtensions.UNIT_SEPARATOR}') AS categories
				FROM mp_blogs b
				JOIN mp_blogitemcategories ic ON ic.itemid = b.itemid
				JOIN mp_blogcategories c ON c.categoryid = ic.categoryid
			...
			{
				new("itemid", NpgsqlTypes.NpgsqlDbType.Integer) { Direction = ParameterDirection.Input, Value = itemId },
				new("currenttime", NpgsqlTypes.NpgsqlDbType.Timestamp) { Direction = ParameterDirection.Input, Value = currentTime },
			};
