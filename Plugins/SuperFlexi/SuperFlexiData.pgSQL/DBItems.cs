...
public static class DBItems
{
		...
		string editRoles)
	{
		...
		var sqlParams = new NpgsqlParameter[]
		{
			...
			new(":createdutc", NpgsqlDbType.Timestamp) { Direction = ParameterDirection.Input, Value = createdUtc },
			new(":lastmodutc", NpgsqlDbType.Timestamp) { Direction = ParameterDirection.Input, Value = lastModUtc },
			new(":viewroles", NpgsqlDbType.Varchar) { Direction = ParameterDirection.Input, Value = viewRoles },
			new(":editroles", NpgsqlDbType.Varchar) { Direction = ParameterDirection.Input, Value = editRoles }
		};
	...
	public static IDataReader GetByIDsWithValues(Guid defGuid, Guid siteGuid, List<int> itemIDs, int pageNumber = 1, int pageSize = 20)
	{
		...
		[
			...
			new("?DefinitionGuid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = defGuid },
			new("?SiteGuid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = siteGuid }
		]);
		...
		bool descending = false)
	{
		...
		var sqlParams = new NpgsqlParameter[]
		{
			...
			new(":searchfield", NpgsqlDbType.Varchar, 50) { Direction = ParameterDirection.Input, Value = searchField },
			new(":moduleguid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = moduleGuid },
		...
		bool descending = false)
	{
		...
		var sqlParams = new NpgsqlParameter[]
		{
			...
			new(":searchterm", NpgsqlDbType.Varchar, 255) { Direction = ParameterDirection.Input, Value = searchTerm },
			new(":searchfield", NpgsqlDbType.Varchar, 50) { Direction = ParameterDirection.Input, Value = searchField },
			new(":defguid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = defGuid },
			new(":siteguid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = siteGuid },
			new(":sortdirection", NpgsqlDbType.Varchar, 4) { Direction = ParameterDirection.Input, Value = descending ? "desc" : "asc" }
		};
	...
	/// </summary>
	/// <param name="itemID"> itemID </param>
	public static IDataReader GetAllForDefinition(
		Guid definitionGuid,
		Guid siteGuid)
	{
		string sqlCommand = """
		...
		{
			new(":defguid", NpgsqlDbType.Varchar) { Direction = ParameterDirection.Input, Value = definitionGuid },
			new(":siteguid", NpgsqlDbType.Varchar) { Direction = ParameterDirection.Input, Value = siteGuid }
		};
	...
	/// <returns></returns>
	public static IDataReader GetByCMSPage(
		Guid siteGuid,
		int pageId)
	{
		var sqlCommand = """
		...
		{
			new(":siteguid", NpgsqlDbType.Varchar) { Direction = ParameterDirection.Input, Value = siteGuid },
			new(":pageid", NpgsqlDbType.Integer) { Direction = ParameterDirection.Input, Value = pageId }
		};
	...
	/// <summary>
	/// Gets Items with their Values for a Module
	/// </summary>
	...
	/// <summary>
	/// Gets a page of Items with their Values for a Module
	/// </summary>
	/// <param name="moduleGuid"></param>
	/// <param name="pageNumber"></param>
	/// <param name="pageSize"></param>
		...
		string sortDirection = "ASC")
	{
		...
		string sqlCommand = $"""
			SELECT i.*
				,ms.SettingValue AS GlobalViewSortOrder
				,Count(i.*) OVER() AS TotalRows
			FROM i7_sflexi_items i
			LEFT JOIN mp_ModuleSettings ms ON ms.ModuleGuid = i.ModuleGuid
			WHERE DefinitionGuid = ?DefGuid 
				AND i.SiteGuid = ?SiteGuid 
				AND ms.SettingName = 'GlobalViewSortOrder' 
			ORDER BY GlobalViewSortOrder {sortDirection}
		...
		var sqlParams = new NpgsqlParameter[]
		{
			new("?DefGuid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = definitionGuid },
			new("?SiteGuid", NpgsqlDbType.Uuid) { Direction = ParameterDirection.Input, Value = siteGuid },
			new("?PageSize", NpgsqlDbType.Integer) { Direction = ParameterDirection.Input, Value = pageSize },
			new("?OffsetRows", NpgsqlDbType.Integer) { Direction = ParameterDirection.Input, Value = offsetRows },
		};
		...
		if (pageSize > -1)
		{
			...
			sqlCommand = $"""
				...
				FROM (
					...
					LIMIT ?PageSize OFFSET ?OffSetRows) pg
				JOIN i7_sflexi_values v ON  v.ItemGuid = pg.ItemGuid
				JOIN i7_sflexi_fields f ON v.FieldGuid = f.FieldGuid
