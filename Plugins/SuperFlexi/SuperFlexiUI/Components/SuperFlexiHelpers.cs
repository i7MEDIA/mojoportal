...
public class SuperFlexiHelpers
{
	...
	{
		var litExtraMarkup = new StringBuilder();
		...
		try
		{
			string settings = string.Format(
				...
				"Admin/ModuleSettings.aspx".ToLinkBuilder().PageId(pageId).ModuleId(moduleId),
				SuperFlexiResources.SettingsLinkLabel);

			if (!string.IsNullOrWhiteSpace(config.MarkupDefinitionName)
				&& config.MarkupDefinitionName != "0")
			{
				if (!config.IsGlobalView
					&& (config.MaxItems == -1 || Item.GetCountForModule(moduleId) < config.MaxItems))
				{
					add = string.Format(
						displaySettings.AddItemLinkFormat,
						"SuperFlexi/Edit.aspx".ToLinkBuilder().PageId(pageId).ModuleId(moduleId),
				...
				if (config.UseHeader)
				{
					header = string.Format(
						...
						"SuperFlexi/EditHeader.aspx".ToLinkBuilder().PageId(pageId).ModuleId(moduleId),
				...
				if (config.UseFooter)
				{
					footer = string.Format(
						...
						"SuperFlexi/EditHeader.aspx".ToLinkBuilder().PageId(pageId).ModuleId(moduleId).AddParam("f", true),
				...
				if (config.AllowImport)
				{
					import = string.Format(
						...
						"SuperFlexi/Import.aspx".ToLinkBuilder().PageId(pageId).ModuleId(moduleId),
				...
				if (config.AllowExport)
				{
					export = string.Format(
						...
						"SuperFlexi/Export.aspx".ToLinkBuilder().PageId(pageId).ModuleId(moduleId),
		...
		catch (FormatException ex)
		{
			...
			log.Error($"Error rendering \"{moduleTitle}\", with moduleID={moduleId}, pageid={pageId}. Error was:\r\n{ex}");
		}
	...
	public static string GetHelpText(string helpKey, ModuleConfiguration config)
	{
			...
			helpKey.ToLower().EndsWith(".html"))
		{
			...
			{
				var path = fileSystem.CombinePath("~/Data/SuperFlexi/Help/", helpKey.Replace("$_FlexiHelp_$", string.Empty));
				helpFile = fileSystem.RetrieveFile(path);

			}
			else if (helpKey.IndexOf("$_SitePath_$") >= 0)
			{
				var path = helpKey.Replace("$_SitePath_$", "~/Data/Sites/" + CacheHelper.GetCurrentSiteSettings().SiteId.ToInvariantString());
				helpFile = fileSystem.RetrieveFile(path);
			}
			else if (helpKey.IndexOf("$_Data_$") >= 0)
			{
				var path = helpKey.Replace("$_Data_$", "~/Data");
				helpFile = fileSystem.RetrieveFile(path);
			}
			...
			{
				var path = fileSystem.CombinePath(config.RelativeSolutionLocation, helpKey);
				helpFile = fileSystem.RetrieveFile(path);
			}
		}
		else
	...
	public static List<MarkupScript> ParseScriptsFromXmlNode(XmlNode childNode)
	{
		...
		if (childNode.Name.ToLower() != "scripts")
		{
	...
	public static string GetPathToFile(ModuleConfiguration config, string path, bool forceHttps = false)
	{
		...
		{
			return path.Replace("$_SitePath_$", $"/Data/Sites/{CacheHelper.GetCurrentSiteSettings().SiteId}/");
		}
		else
		{
	...
	public static void SetupScripts(
		...
		Control control)
	{
		string scriptRefFormat = "\n<script data-loader=\"SuperFlexiSolution\" data-name=\"{1}\" src=\"{0}\"></script>";
		string rawScriptFormat = "\n<script data-loader=\"SuperFlexiSolution\" data-name=\"{1}\">\n{0}\n</script>";

		foreach (MarkupScript script in markupScripts)
		{
			var sbScriptText = new StringBuilder();
			var sbScriptName = new StringBuilder();
			...
			switch (script.Position)
			{
				...
				case "aboveMarkupDefinition":
					...
					if (control.FindControlRecursive(scriptName) == null)
					{
						...
						{
							var aboveLit = new LiteralControl
							{
								ID = scriptName,
				...
				case "belowMarkupDefinition":
					...
					if (control.FindControlRecursive(scriptName) == null)
					{
						...
						{
							var belowLit = new LiteralControl
							{
								ID = scriptName,
								Text = sbScriptText.ToString()
							};
	...
	public static void SetupStyle(
		...
		Control control)
	{
		string styleLinkFormat = "\n<link rel=\"stylesheet\" data-name=\"{1}\" media=\"{2}\" href=\"{0}\" >";
		...
		foreach (MarkupCss style in markupCss)
		{
			...
			var theLiteral = new LiteralControl
			{
	...
	internal static List<MarkupCss> ParseCssFromXmlNode(XmlNode childNode)
	{
		...
		if (childNode.Name.ToLower() != "styles")
		{
			return markupCss;
		}
		...
		foreach (XmlNode child in childNode)
		{
			if (child.Name.ToLower() == "style")
			{
				...
				if (childAttrs["href"] != null)
				{
					...
					{
						Url = childAttrs["href"].Value,
						Media = media
					};
					if (childAttrs["renderAboveSSC"] != null)
					{
						style.RenderAboveSSC = Convert.ToBoolean(childAttrs["renderAboveSSC"].Value);
					}
				...
				if (!string.IsNullOrWhiteSpace(child.InnerText))
				{
					var raw = new MarkupCss
					{
						CSS = child.InnerText.Trim(),
						Media = media
					};
					if (!string.IsNullOrWhiteSpace(name))
					{
	...
	public static void ParseSearchDefinition(XmlNode searchNode, Guid fieldDefinitionGuid, Guid siteGuid)
	{
		...
		{
			var emptySearchDef = false;
			var searchDefExists = true;
			var searchDef = SearchDef.GetByFieldDefinition(fieldDefinitionGuid);

			if (searchDef is null)
			{
