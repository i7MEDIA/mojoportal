...
public partial class FeedAggregatePage : Page
{
	...
	private void RenderRss()
	{
		...
		var feed = new Argotic.Syndication.RssFeed();
		...
		foreach (DataRowView row in dv)
		{
			...
			if (confirmed)
			{
				var item = new RssItem
				{
					...
					Link = new Uri(row["Link"].ToString())
				};
	...
	private void LoadSettings()
	{
		...
		if (securityBypassGuid != Guid.Empty 
			&& securityBypassGuid == WebConfigSettings.InternalFeedSecurityBypassKey)
		{
			bypassPageSecurity = true;
		}

		if (bypassPageSecurity
