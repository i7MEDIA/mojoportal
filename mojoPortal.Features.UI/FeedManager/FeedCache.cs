...
public static class FeedCache
{
	...
	{
		if (d == null || d == DateTime.MinValue || d == DateTime.MaxValue)
		{
	...
	)
	{
		...
		try
		{
			...
			var dtFeeds = Business.RssFeed.GetFeeds(moduleId);
			var siteRoot = SiteUtils.GetNavigationSiteRoot();
			var secureSiteRoot = SiteUtils.GetSecureNavigationSiteRoot();

			foreach (DataRow dr in dtFeeds.Rows)
			{
				var feedGuid = new Guid(dr["ItemGuid"].ToString());
				var feedId = Convert.ToInt32(dr["ItemID"]);
				var feedUrl = dr["RssUrl"].ToString();

				if (feedUrl.StartsWith("~/"))
				{
					var useHttps = (siteSettings != null && !siteSettings.UseSslOnAllPages) || HttpContext.Current.Request.IsSecureConnection;
					feedUrl = WebUtils.ResolveServerUrl(feedUrl, useHttps);
				}

				var publishByDefault = Convert.ToBoolean(dr["PublishByDefault"]);
				var countOfPreservedEntries = Convert.ToInt32(dr["TotalEntries"]);
				...
				var entriesAdded = countOfPreservedEntries;
				...
				try
				{
					...
					{
						if (gsFeed.Resource is Argotic.Syndication.RssFeed rssFeed)
						{
					...
					if (gsFeed.Format == SyndicationContentFormat.Atom)
					{
						...
						{
							var entryLink = string.Empty;
							var entryAuthor = new StringBuilder();
							var comma = string.Empty;
							var itemPubDate = EnsureDate(atItem.UpdatedOn);
	...
	)
	{
		...
		try
		{
			...
			try
			{
				var gsFeed = GenericSyndicationFeed.Create(new Uri(FormatFeedUrl(feedUrl, siteRoot, secureSiteRoot)));
					...
					if (gsFeed.Resource is Argotic.Syndication.RssFeed rssFeed)
					{
						foreach (RssItem rssItem in rssFeed.Channel.Items)
						{
							if ((rssItem.PublicationDate >= cutoffDate) || (maxDaysOld == 0))
							{
								if ((entriesAdded < maxEntriesPerFeed) || (maxEntriesPerFeed == 0))
								{
									var entryBlob = rssItem.Title + rssItem.Link.ToString();
									var entryHash = GetEntryHash(entryBlob);
					...
					foreach (AtomEntry atItem in atomFeed.Entries)
					{
						...
						if ((atItem.PublishedOn >= cutoffDate) || (maxDaysOld == 0))
						{
							if ((entriesAdded < maxEntriesPerFeed) || (maxEntriesPerFeed == 0))
							{
								...
								string entryBlob = $"{atItem.Title}{entryLink}";
								int entryHash = GetEntryHash(entryBlob);
	...
	)
	{
		...
		{
			return Business.RssFeed.UpdateEntry(
				 moduleGuid,
				 entryTitle,
				 entryAuthor,
				 feedUrl,
