...
public partial class FeedEditPage : NonCmsBasePage
{
	...
	protected FeedManagerConfiguration config = new();
	...
	{
		var feed = new RssFeed(ModuleId, ItemId);
	...
	private void btnUpdate_Click(object sender, EventArgs e)
	{
		...
		int.TryParse(txtSortRank.Text, out int sortRank);
		...
		if (feed.Save())
		{
			...
			string rssFriendlyUrl = Invariant($"aggregator{ModuleId}rss.aspx");

			if (!FriendlyUrl.Exists(siteSettings.SiteId, rssFriendlyUrl))
			{
				var friendlyUrl = new FriendlyUrl
				{
	...
	private void PopulateLabels()
	{
		...
		lnkNewFeed.NavigateUrl = "FeedManager/FeedEdit.aspx".ToLinkBuilder().PageId(PageId).ModuleId(ModuleId).ItemId(-1).ToString();
