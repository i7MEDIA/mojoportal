	...
	public partial class ForumPostEdit : NonCmsBasePage
	{
		...
		private void PopulateControls()
		{
			...
			else
			{
				...
				{
					Title = SiteUtils.FormatPageTitle(siteSettings, $"{forum.Title} - {thread.Subject.RemoveMarkup()}");
				}
			...
			if (thread != null)
			{
				lblThreadDescription.Text = thread.Subject.RemoveMarkup();
			}
		}
		...
		{
			var thread = new ForumThread(threadId, postId);
