      ...
      <ItemTemplate >
         <tr class="modulerow">
            <td headers='<%# tdSubscribedHead.ClientID %>' id="tdSubscribed" runat="server" enableviewstate='<%# ShowSubscribeCheckboxes %>' class="txtmed padded fsubscribe" Visible='<%# Request.IsAuthenticated %>' > 
				...
				<asp:HyperLink ID="lnkNotify" runat="server" EnableViewState="false" Visible='<%# !ShowSubscribeCheckboxes %>' ImageUrl='<%# ImageSiteRoot + "/Data/SiteImages/email.png"  %>' NavigateUrl='<%# notificationUrl + "#forum" + Eval("ItemID") %>' 
