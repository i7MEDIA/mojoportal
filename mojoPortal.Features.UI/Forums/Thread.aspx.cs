...
namespace mojoPortal.Web.ForumUI
{
	public partial class ForumThreadView : mojoBasePage
	{
		...
		private void PopulateControls()
		{
			...
			{
				Title = SiteUtils.FormatPageTitle(siteSettings, CurrentPage.PageName + " - " + FormatTitle(thread).RemoveMarkup());
			}
			else
			{
				Title = SiteUtils.FormatPageTitle(siteSettings, FormatTitle(thread).RemoveMarkup());
			}
