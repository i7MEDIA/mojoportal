	...
	public static class DBBlog
	{
		...
		public static IDataReader GetBlogsByPage(int siteId, int pageId)
		{
			var sqlCommand = $"""
				SELECT
					...
					u.Email,
					u.AvatarUrl,
					GROUP_CONCAT (c.Category SEPARATOR '{UnitSeparatorExtensions.UNIT_SEPARATOR}') AS Categories
				FROM
					mp_Blogs b
					JOIN mp_BlogItemCategories ic ON ic.ItemID = b.ItemID
					JOIN mp_BlogCategories c ON c.CategoryID = ic.CategoryID
					JOIN mp_Modules m ON b.ModuleID = m.ModuleID
					...
					LEFT OUTER JOIN mp_Users u ON b.UserGuid = u.UserGuid
				WHERE
					p.SiteID = ?SiteID
		...
		public static IDataReader GetBlogMonthArchive(int moduleId, DateTime currentTime)
		{
			...
			var sqlParams = new MySqlParameter[] {
				...
				new("?CurrentDate", MySqlDbType.DateTime) { Direction = ParameterDirection.Input, Value = currentTime }
			};
		...
		public static IDataReader GetSingleBlog(int itemId, DateTime currentTime)
		{
			var sqlCommand = $"""
				SELECT
					...
					(
						...
						AND (b2.EndDate IS NULL OR b2.EndDate > ?CurrentTime)
						AND (b2.StartDate > b.StartDate)
					...
					COALESCE(
						(
							...
							AND (b6.EndDate IS NULL OR b6.EndDate > ?CurrentTime)
							AND (b6.StartDate > b.StartDate)
							AND b6.ModuleID = b.ModuleID
							ORDER BY b6.StartDate
							LIMIT 1
						),
						-1
					) AS NextItemID,
					...
					(
						...
						AND b3.ModuleID = b.ModuleID
						AND b3.ItemUrl IS NOT NULL
						AND (b3.ItemUrl <> '')
						ORDER BY b3.StartDate DESC
						LIMIT 1
					) AS PreviousPost,
			...
			{
				new("?ItemID", MySqlDbType.Int32) { Direction = ParameterDirection.Input, Value = itemId },
				new("?CurrentTime", MySqlDbType.DateTime) { Direction = ParameterDirection.Input, Value = currentTime },
			};
